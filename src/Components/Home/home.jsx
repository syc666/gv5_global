...
class Commonli extends Component {
	goOther(event){
		...
		window.open("http://192.168.44.7:3001/#/earth/"+hrefCon+"");   
	}
	render(){
		return (
...
class Allthemes extends Component {
	constructor(props) {  
        super(props);  
        this.state = {searchStr: 'aa',data:Data.details1};  
    }	
	tabTheme(msg,event){
				...
				<div>			
					{
						this.state.data.map((val) =>
							<Commonli key={val.titleDesc.toString()} themesliImg={val.themesliImg} titleDesc={val.titleDesc} letterDesc={val.letterDesc} connentDesc={val.connentDesc} author={val.author} time={val.time}></Commonli>
						)
					}
				</div>
			</div>		
